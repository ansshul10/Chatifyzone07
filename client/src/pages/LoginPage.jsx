...
import { 
    ...
    FaSatellite, FaMicrochip, FaNetworkWired
} from "react-icons/fa";
...
/**
 * CHATIFY PROFESSIONAL QUANTUM BUILD v3.2.1
...
export default function LoginPage() {
    ...
    const handleFutureFeature = useCallback((gateway) => {
        toast.error(`GATEWAY_LOCKED: ${gateway} integration is scheduled for v4.0.`, {
            style: {
                background: '#0a0a0a',
                color: '#ff0000',
    ...
    const handleLoginSubmit = async (ev) => {
        ...
        const statusId = toast.loading("Syncing with Central Intelligence...");

        try {
            const response = await axiosInstance.post('/auth/login', { email, password });
            toast.success(`Encrypted Link Established. Welcome.`, { id: statusId });
            setUser(response.data.user);
            setTimeout(() => navigate('/'), 1000);
        } catch (error) {
            const msg = error.response?.data?.error || "Neural Handshake Rejected.";
            setFormError(msg);
                ...
                >
                    ...
                    <div className="bg-[#111]/95 backdrop-blur-2xl p-8 md:p-10 rounded-[2rem] border border-white/5 relative z-10">
                        ...
                        <motion.div variants={elementFade} className="text-center mb-8">
                            ...
                            <p className="text-gray-600 text-[9px] mt-2 font-bold uppercase tracking-[0.4em]">System Authorization</p>
                        </motion.div>
                        ...
                        <form onSubmit={handleLoginSubmit} className="space-y-5">
                            <motion.div variants={elementFade} className="space-y-2">
                                ...
                                <div className="relative group">
                                    ...
                                    <input 
                                        type="email" 
                                        placeholder="nexus@chatify.io"
                                        required
                                        className="w-full pl-11 pr-4 py-3.5 bg-black/40 border border-white/5 rounded-xl outline-none focus:border-[#FF0000] transition-all text-xs font-medium placeholder:text-gray-800"
                                        value={email}
                            ...
                            <motion.div variants={elementFade} className="space-y-2">
                                <div className="flex justify-between items-center px-1">
                                    ...
                                    <button type="button" onClick={() => handleAdminBlock("Recovery")} className="text-[9px] text-gray-700 hover:text-[#FF0000] font-black uppercase">Lost Key?</button>
                                </div>
                                <div className="relative group">
                                    ...
                                    <input 
                                        ...
                                        placeholder="••••••••••••"
                                        required
                                        className="w-full pl-11 pr-11 py-3.5 bg-black/40 border border-white/5 rounded-xl outline-none focus:border-[#FF0000] transition-all text-xs font-mono tracking-widest"
                                        value={password}
                            ...
                            <motion.button 
                                ...
                                whileTap={{ scale: 0.99 }}
                                disabled={isLoading}
                                className={`w-full py-3.5 rounded-xl font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 transition-all ${isLoading ? 'bg-gray-900 text-gray-700' : 'bg-[#FF0000] text-white hover:brightness-110 shadow-lg shadow-[#FF0000]/10'}`}
                            >
                        ...
                        <motion.p variants={elementFade} className="mt-8 text-center text-gray-600 text-[10px] font-black uppercase tracking-widest">
                            No Identity Matrix? 
                            <Link to="/register" className="text-[#FF0000] ml-2 hover:underline">Register</Link>
                        </motion.p>
                    </div>
                </motion.div>
            </main>
